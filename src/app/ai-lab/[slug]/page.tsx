...
export async function generateStaticParams() {
  const items = getAllContent('ai-lab')
  return items.map((item) => ({ slug: item.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const data = getContentBySlug('ai-lab', slug)
  if (!data) return { title: 'Not Found' }
  return { title: `${data.meta.title} — Jim Zhou` }
}

export default async function AILabDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const data = getContentBySlug('ai-lab', slug)
  if (!data) notFound()
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/ai-lab" className="text-sm text-gray-500 hover:text-teal-400 transition-colors mb-6 inline-block">
        &larr; Back to AI Lab
      </Link>
